...
async function fetchUSGSWater() {
  const url = `https://api.waterdata.usgs.gov/ogcapi/v0/collections/monitoring-locations/items?bbox=${DEFAULT_LON - 0.5},${DEFAULT_LAT - 0.5},${DEFAULT_LON + 0.5},${DEFAULT_LAT + 0.5}&limit=50&f=json`;
  const data = await fetchWithTimeout(url);
