...
async function collectOpenSourceSignals({ sourceIds = [], concurrency = 4, includeAll = false } = {}) {
  const requested = sourceIds.length ? sourceIds : (includeAll ? SOURCE_REGISTRY.map((s) => s.id) : CORE_SOURCE_IDS);
...
module.exports = { SOURCE_REGISTRY, CORE_SOURCE_IDS, collectOpenSourceSignals, fetchWithTimeout };
