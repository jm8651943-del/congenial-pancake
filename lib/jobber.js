...
async function exchangeCode(code, verifier) {
  ...
  if (!payload.access_token || !payload.refresh_token) {
    throw new Error("Jobber token response did not include required tokens");
  }
...
function getDisconnectMutation() {
  return [
...
const QUERIES = {
  account: "query GetAccount { account { id name } }",
  clients: "query GetClients { clients(first: 25) { nodes { id firstName lastName companyName updatedAt billingAddress { city province postalCode } } pageInfo { hasNextPage endCursor } totalCount } }",
  jobs: "query GetJobs { jobs(first: 25) { nodes { id jobNumber title jobStatus startAt endAt total uninvoicedTotal updatedAt client { id name } property { id name } } pageInfo { hasNextPage endCursor } } }",
};
