...
const COLLECTIONS = {
  clients: `clients(first: 25) {
    nodes { id firstName lastName companyName updatedAt }
    pageInfo { hasNextPage endCursor }
    totalCount
  }`,
  jobs: `jobs(first: 25) {
    nodes { id jobNumber title jobStatus startAt endAt total uninvoicedTotal updatedAt client { id } property { id name } }
    pageInfo { hasNextPage endCursor }
  }`,
  quotes: `quotes(first: 25) {
    nodes { id quoteNumber title quoteStatus createdAt sentAt updatedAt amounts { total } client { id } property { id name } }
    pageInfo { hasNextPage endCursor }
  }`,
  requests: `requests(first: 25) {
    nodes { id title requestStatus createdAt updatedAt source client { id } property { id name } }
    pageInfo { hasNextPage endCursor }
  }`,
  visits: `visits(first: 25) {
    nodes { id title visitStatus startAt endAt isComplete client { id } job { id jobNumber } property { id name } }
    pageInfo { hasNextPage endCursor }
  }`,
  invoices: `invoices(first: 25) {
    nodes { id invoiceNumber invoiceStatus issuedDate updatedAt subject amounts { total } }
    pageInfo { hasNextPage endCursor }
  }`,
};
